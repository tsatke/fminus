...
#[test]
fn test_string() {
    let value = fminus::eval_str("'hello'").expect("expected something");
    assert_eq!(value.string().expect("expected a string"), "hello");
}

#[test]
fn test_bool() {
    let value = fminus::eval_str("true").expect("expected something");
    assert_eq!(value.boolean().expect("expected a bool"), true);

    let value = fminus::eval_str("false").expect("expected something");
    assert_eq!(value.boolean().expect("expected a bool"), false);
}

#[test]
fn test_list() {
    let value = fminus::eval_str("[1, '2', [false, true]]").expect("expected something");
